...
Supply-chain trust assessment (reason about the provided signature/
attestation status — do not attempt to verify it yourself)
Release-over-release delta analysis, using the delta_status already
assigned to each finding — do not infer freshness yourself
Exception/suppression awareness — reason about existing risk-acceptance
...
Fix availability
Severity
Delta status — a finding new to this release outweighs an
